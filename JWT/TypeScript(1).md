...
자바스크립트(동적 타이핑)와는 달리,  타입스크립트는 **정적 타입의 컴파일 언어**이며 타입스크립트 컴파일러 또는 바벨(Babel)을 통해 자바스크립트 코드로 변환됩니다. 
<details>
...
- **오버로딩**: 자바스크립트의 경우 전통적인 오버로딩이 되지않고 맨 아랫것을 기준으로 실행되는 반면, TypeScript는 함수 오버로딩을 허용합니다. TypeScript에서는 함수 오버로딩을 사용하여 동일한 함수명으로 여러 다양한 매개변수 형태를 처리할 수 있습니다.
  - (애초에 자바스크립트는 동적 타이핑이기 때문에 오버로딩의 필요성이 낮은데, 오버로딩같이 동작하도록 구현할 수는 있음)
<details>
<summary>오버로딩이란?</summary>
<div markdown="1">
...
## Any
TypeScript에서는 변수를 선언할 때 명시적인 타입을 지정하는 것이 권장되지만,
타입을 명시적으로 지정하지 않고, 컴파일러가 컨텍스트에서 이를 확인할 수 없는 경우 Typescript는 해당 변수를 암시적인 `any` 타입으로 간주합니다.

어떨때 쓰냐: 변수 타입 뭔지 모를때. (사용자 입력이나 API요청(근데 보통은 앎.) 

타입스크립트의 이점이 사라집니다. 컴파일러가 컨텍스트에서 이를 확인할 수 없는 경우 타입명시적으로 지정하려고 타스 쓰는건데 any??
any 사용할바엔 javascript쓰시는게 낫습니다.

### NoImplicitAny

**`NoImplicitAny`**는 TypeScript의 컴파일 옵션 중 하나로, 암시적인 **`any`** 타입을 허용하지 않도록 강제하는 옵션입니다.
